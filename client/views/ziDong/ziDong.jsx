...
export default class Tab extends React.Component {
	render() {
		return (
			<div className='ziDong'>
				<div className="nav flex">
					<div className="navText flex"><Link to="/creative">首页</Link>&nbsp;>&nbsp;<Link to="/creative/itcontent">优质帖子</Link></div>
				</div>
				<div className="boxIt">
					<div className='boxItTop'>掌测AppSTest|自动化测试</div>
					<div className="itTop">
						<div className="textContentOne">
							<div>自动化测试是把以人为驱动的测试行为转化为机器执行的一种过程。通常，在设计了测试用例并通过评审之后，由测试人员根据测试用例中描述的规程一步步执行测试，得到实际结果与期望结果的比较。在此过程中，为了节省人力、时间或硬件资源，提高测试效率，便引入了自动化测试的概念。</div>
							<div className='ziImgOne'>
								<div>一、实施自动化测试之前需要对软件开发过程进行分析，以观察其是否适合使用自动化测试。通常需要同时满足以下条件：</div>
								<div className='xuContent'>
									<div>1) 需求变动不频繁</div>
									<div>测试脚本的稳定性决定了自动化测试的维护成本。如果软件需求变动过于频繁，测试人员需要根据变动的需求来更新测试用例以及相关的测试脚本，而脚本的维护本身就是一个代码开发的过程，需要修改、调试，必要的时候还要修改自动化测试的框架，如果所花费的成本不低于利用其节省的测试成本，那么自动化测试便是失败的。</div>
									<div>项目中的某些模块相对稳定，而某些模块需求变动性很大。我们便可对相对稳定的模块进行自动化测试，而变动较大的仍是用手工测试。</div>
								</div>
								<div className='xuContent'>
									<div>2) 项目周期足够长</div>
								...
								<div className='xuContent'>
									...
									<div>如果费尽心思开发了一套近乎完美的自动化测试脚本，但是脚本的重复使用率很低，致使其间所耗费的成本大于所创造的经济价值，自动化测试便成为了测试人员的练手之作，而并非是真正可产生效益的测试手段了</div>
									<div>另外，在手工测试无法完成，需要投入大量时间与人力时也需要考虑引入自动化测试。比如性能测试、配置测试、大数据量输入测试等。</div>
								</div>
							</div>
							<div className='ziImgOne'>
								<div>二、通常适合于软件测试自动化的场合：</div>
								<div className='xuContent'>
									<div>(1)回归测试，重复单一的数据录入或是击键等测试操作造成了不必要的时间浪费和人力浪费；</div>
									<div>(2)此外测试人员对程序的理解和对设计文档的验证通常也要借助于测试自动化工具；</div>
									<div>(3)采用自动化测试工具有利于测试报告文档的生成和版本的连贯性；</div>
									<div>(4)自动化工具[3]  能够确定测试用例的覆盖路径，确定测试用例集对程序逻辑流程和控制流程的覆盖。</div>
									<div>随着测试流程的不断规范以及软件测试技术的进一步细化，软件测试自动化已经日益成为一支不可忽视的力量。能否借助于这支外在力量以及如何借助于这支力量来规范企业测试流程、提高特定测试活动的效率，正是本期所要讨论的话题。</div>
							...
							<div className='ziImgOne'>
								<div>三、自动化测试与软件开发过程从本质上来讲是一样的，无非是利用自动化测试工具（相当于软件开发工具），经过对测试需求的分析（软件过程中的需求分析），设计出自动化测试用例（软件过程中的需求规格），从而搭建自动化测试的框架（软件过程中的概要设计），设计与编写自动化脚本（详细设计与编码），测试脚本的正确性，从而完成该套测试脚本（即主要功能为测试的应用软件）。</div>
								<div className='xuContent'>
									<div>1) 自动化测试需求分析。</div>
									...
									<div>所谓自动化测试框架便是像软件架构一般，定义了在使用该套脚本时需要调用哪些文件、结构，调用的过程，以及文件结构如何划分。</div>
									<div>而根据自动化测试用例，我们很容易能够定位出自动化测试框架的典型要素：</div>
									<div>a. 公用的对象。</div>
